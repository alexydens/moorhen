...
static inline void _printf(void (*_putc)(char), const char *format, ...) {
  va_list args;
...
#else
          /*
           * Without SSE, we can't cast to a float, so we need to cast to an int
           * if we want to discard the argument.
           */
